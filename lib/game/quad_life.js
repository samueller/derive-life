...
const placeSeedTransposed = (seed, num, row, col) => {
	...
	// for (let i = 0; i < seed.height; i++)
	// 	for (let j = 0; j < seed.width; j++)
	// 		if (seed.cells[seed.width * i + j])
	// 			grid[seed.height * (row + j) + col + i] = num
}
	...
	runsThread.onmessage = function (e) {
		switch (e.data[0]) {
			...
			case Message.initialScoringUpdate:
				...
				seedsInitialized.set(e.data[1] + 1)
				break
			...
			case Message.generationCompleted:
				...
				topRelativeFitness.set(
					e.data[1].reduce((sum, seed) => sum + seed.score, 0) /
						e.data[1].length
				)
				console.log('Top seed score', e.data[1][0].score)
