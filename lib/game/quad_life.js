...
const startWorkers = () => {
    console.log("quadLife: start game with " + window.navigator.hardwareConcurrency + " background threads");
    workerManager = new Worker('./lib/game/WorkerManager.js', { type: "module" });
    workerManager.postMessage([ManagerMessages.startWorkers, ManagerMessages.initialRun, 10]);
