...
const placeSeedTransposed = (seed, num, row, col) => {
	for (let i = 0; i < seed.width; i++)
		for (let j = 0; j < seed.height; j++)
			if (seed.cells[seed.height * j + i])
				grid[cols * (row + i) + col + j] = num
}
...
const placeSeeds = () => {
	placeSeedTransposed(
		currentSeed1,
		1,
		Math.round(rows / 4 - currentSeed1.height / 2),
		Math.round((cols * 3) / 4 - currentSeed1.width / 2)
	)
	placeSeed(
		currentSeed2,
		2,
		Math.round(rows / 4 - currentSeed2.height / 2),
		Math.round(cols / 4 - currentSeed2.width / 2)
	)
	placeSeed(
		currentSeed3,
		3,
		Math.round((rows * 3) / 4 - currentSeed3.height / 2),
		Math.round(cols / 4 - currentSeed3.width / 2)
	)
	placeSeed(
		currentSeed4,
		4,
		Math.round((rows * 3) / 4 - currentSeed4.height / 2),
		Math.round((cols * 3) / 4 - currentSeed4.width / 2)
	)
}
	...
	runsThread.onmessage = function (e) {
		switch (e.data[0]) {
			...
			case Message.generationCompleted:
				...
				console.log('Top seed', e.data[1][0].score)
				updateTopSeeds(...e.data[1])
				break
