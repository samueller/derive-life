...
const coords = (row, col) => row * cols + col

const findNeighbors = (row, col) => {
	const result = []
	if (grid[coords(row, col)] != 0) result.push(grid[coords(row, col)])
	if (grid[coords(row, col)] != 0) result.push(grid[coords(row, col)])
	if (grid[coords(row, col)] != 0) result.push(grid[coords(row, col)])
	if (grid[coords(row, col)] != 0) result.push(grid[coords(row, col)])
	if (grid[coords(row, col)] != 0) result.push(grid[coords(row, col)])
...
const step = () => {
	for (let i = 0; i < rows; i++)
		for (let j = 0; j < cols; j++) {
			...
			if (grid[coords(i, j)] == 0 && neighbors.length == 3)
				grid[coords(i, j)] = pickColor(neighbors)
		}
}
